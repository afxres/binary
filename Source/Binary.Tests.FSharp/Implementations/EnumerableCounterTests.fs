...
let testVariable<'a> () =
    ...
    Assert.Equal("GenericsConverter`2", converterType.Name)
    ...
    Assert.Equal("GenericsVariableEncoder`2", encoderType.Name)
    ()
...
let testConstant<'a> (collection : 'a) (counterName : string) (count : int) =
    ...
    let encoder = encoderField.GetValue(converter)
    let encoderType = encoder.GetType()
    Assert.Equal("GenericsConstantEncoder`2", encoderType.Name)
