...
[<Fact>]
let ``Append (default constructor, zero)`` () =
    let error = Assert.Throws<ArgumentException>(fun () ->
        let mutable allocator = Allocator()
        AllocatorHelper.Append(&allocator, 0, null :> obj, fun a b -> ())
        ())
...
[<Fact>]
let ``Append (append some then, zero)`` () =
    let mutable flag = 0
    let error = Assert.Throws<ArgumentException>(fun () ->
        let mutable allocator = Allocator()
...
[<Fact>]
let ``Append (limited to zero, zero)`` () =
    let error = Assert.Throws<ArgumentException>(fun () ->
