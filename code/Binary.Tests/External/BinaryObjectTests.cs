...
{
    private delegate object Create(ImmutableArray<ImmutableArray<byte>> items, out int error);
    ...
    public void CreateItemCountLimited()
    {
        ...
        var da = create.Invoke([.. a.Select(x => Encoding.UTF8.GetBytes(x.ToString()).ToImmutableArray())], out var ea);
        var db = create.Invoke([.. b.Select(x => Encoding.UTF8.GetBytes(x.ToString()).ToImmutableArray())], out var eb);
