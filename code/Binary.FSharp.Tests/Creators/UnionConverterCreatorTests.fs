...
type UnionConverterCreatorTests() =
    ...
    member __.``Get Converter Of FSharp List``() =
        let creatorType =
            typeof<GeneratorBuilderFSharpExtensions>.Assembly.GetTypes() |> Array.filter (fun x -> x.Name = "UnionConverterCreator") |> Array.exactlyOne
        let creator = Activator.CreateInstance creatorType :?> IConverterCreator
        let context =
            { new IGeneratorContext with
                member __.GetConverter _ = raise (NotSupportedException())
            }
