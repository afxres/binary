...
type PlaceholderAttribute() = inherit Attribute()
...
type BadClassIndexerWithNamedKey() =
    [<Placeholder>]
    [<NamedKey("Indexer???")>]
    member __.Some with private get (_ : int) = String.Empty and set _ _ = ()
...
type BadValueIndexerWithTupleKey =
    [<TupleKey(1)>]
    [<Placeholder>]
    member __.Just with set (_ : single) (_ : double) = ()
...
type BadClassIndexerWithConverter() =
    [<Placeholder>]
    [<Converter(null)>]
    member __.Maybe with get (_ : string) = 0
...
type BadValueIndexerWithConverterCreator =
    ...
    [<Placeholder>]
    member __.None with get (_ : single) = double 0 and private set _ _ = ()
...
type BadClassWithMultipleAttributesOnAllElements() =
    ...
    [<Converter(null)>]
    [<ConverterCreator(null)>]
    member __.``Set Only Property`` with set (_ : Guid) = ()

    [<NamedKey("2")>]
    [<TupleKey(-10)>]
    [<Converter(null)>]
    [<ConverterCreator(null)>]
    member __.Item with set (_ : single) (_ : double) = ()
...
type ClassAsNamedObjectWithSetOnlyIndexer(alpha : double, text : string) =
    ...
    [<Placeholder>]
    member __.``Index What`` with set (_ : int) (_ : obj) = ()
...
type ValueAsTupleObjectWithIndexer =
    ...
    [<Placeholder>]
    member __.``This is an indexer`` with get (_ : string) = obj() and set _ _ = raise (NotSupportedException())

    [<TupleKey(1)>]
    [<Placeholder>]
    member me.Bravo with get () = me.b and set x = me.b <- x

    [<Placeholder>]
    [<TupleKey(0)>]
    member me.Delta with get () = me.t and set x = me.t <- x
...
type AttributeWithIndexerTests() =
    ...
    static member ``Data Multiple Integration`` = [|
        [| typeof<BadClassWithMultipleAttributesOnAllElements> |]
    |]
