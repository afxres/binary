...
let ``Constructor (argument out of range)`` (limits: int) =
    ...
    let parameterName =
        constructors |> Array.map (fun x -> x.GetParameters() |> Array.last) |> Array.map (fun x -> x.Name) |> Array.distinct |> Array.exactlyOne
...
let ``Constructor (argument underlying allocator null)`` () =
    ...
    let parameterName =
        constructors |> Array.map (fun x -> x.GetParameters() |> Array.head) |> Array.map (fun x -> x.Name) |> Array.distinct |> Array.exactlyOne
