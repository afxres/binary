...
type FakeObjectGenerator(converters: IReadOnlyDictionary<Type, IConverter>) =
    interface IGenerator with
        member __.GetConverter t = converters.[t]

type GeneratorObjectConverterTests() =
    static let CreateConverter () =
        let sequences = [ typeof<int>, FakeObjectConverter<int>() :> IConverter; typeof<string>, FakeStringConverter() :> IConverter ]
    ...
    static let rec InvokeWithInsufficientExecutionStack (action: Action) =
        let status =
            try
                ...
                None
            with :? InsufficientExecutionStackException as x ->
                Some x
        match status with
        | None -> InvokeWithInsufficientExecutionStack action
        | _ -> action.Invoke()
        ()
    ...
    static member ``Encode Arguments``: (obj array) seq = seq {
        ...
        yield [|
            Action<obj>(fun x -> let mutable allocator = Allocator() in CreateConverter().EncodeWithLengthPrefix(&allocator, x))
