...
type Delta =
    ...
    member me.C
        with get () = me.c
        and set value = me.c <- value

    member me.A
        with get () = me.a
        and set value = me.a <- value

    member me.B
        with get () = me.b
        and set value = me.b <- value
...
type ClassMultipleFullConstructorMultipleConstructorThenMembersValidViaMembers(first: single, second: double, third: string, fourth: Guid) =
    ...
    new(fourth: Guid, third: string, second: double, first: single) =
        ClassMultipleFullConstructorMultipleConstructorThenMembersValidViaMembers(first, second, third, fourth)

    new(fourth: Guid, second: double, third: string, first: single) =
        ClassMultipleFullConstructorMultipleConstructorThenMembersValidViaMembers(first, second, third, fourth)
